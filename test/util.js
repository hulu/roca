 ...
 * @param {string} workingDir - the absolute path to the directory in which Roca should be executed
 ...
 */
exports.rocaInDir = async function(workingDir) {
    let stdout = jest.spyOn(process.stdout, "write").mockImplementation(() => true);
    let cwd = jest.spyOn(process, "cwd").mockImplementation(() => workingDir);
